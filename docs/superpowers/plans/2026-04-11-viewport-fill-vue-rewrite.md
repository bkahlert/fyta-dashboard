...
<!-- src/App.vue -->
<template>
  <div class="flex items-center justify-center h-screen text-primary text-2xl font-bold">
...
console.assert(JSON.stringify(calcGrid(23, 1024, 720)) === JSON.stringify({cols:6,rows:4}), '23 plants test failed')
// 1 plant → 1×1
console.assert(JSON.stringify(calcGrid(1, 1024, 720)) === JSON.stringify({cols:1,rows:1}), '1 plant test failed')
// 4 plants in square → 2×2
console.assert(JSON.stringify(calcGrid(4, 800, 800)) === JSON.stringify({cols:2,rows:2}), '4 plants test failed')
...
<template>
  <header class="navbar bg-base-200 border-b border-base-300 min-h-12 px-4 shrink-0">
    ...
    <div class="flex-none flex items-center gap-3">
      <span v-if="lastUpdated" class="text-xs text-base-content/50 hidden sm:inline">
        {{ timeAgo }}
      </span>
      <button
...
<template>
  ...
  >
    <span class="badge badge-ghost badge-sm">{{ plants.length }} plant{{ plants.length !== 1 ? 's' : '' }}</span>
  ...
  <div class="card shadow overflow-hidden" :class="cardBorderClass">
    ...
    <div class="card-body p-2 gap-1 min-h-0 flex flex-col">
      ...
      <div class="flex items-center gap-1 min-w-0">
        ...
        <span class="font-bold truncate" :class="[moistureColorClass, tier === 'micro' ? 'text-xs' : 'text-xs']">
      ...
      <div class="flex flex-col gap-0.5 mt-auto">
        <div v-for="[icon, code] in sensors" :key="icon" class="flex items-center gap-1">
          <span v-if="tier !== 'micro'" class="text-xs w-3 shrink-0 leading-none">{{ icon }}</span>
          ...
          <span class="text-xs shrink-0 w-10 text-right" :class="labelColorClass(code)">
            {{ STATUS[code]?.label ?? '' }}
          </span>
        </div>
      </div>
...
const MOISTURE_HEADLINE = {
  0: 'No sensor', 1: 'Needs Water', 2: 'Water Soon',
  3: 'Well Watered', 4: 'Slightly Wet', 5: 'Overwatered',
}
