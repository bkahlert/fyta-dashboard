# FYTA Home Assistant — Claude Notes

## Context

Lovelace dashboard for FYTA plant data, parked here while the Vue app is the active workstream.
...
All scripts live in `home-assistant/scripts/` and require `HA_TOKEN` in the environment.

| Script                       | Purpose                                     |
| ---------------------------- | ------------------------------------------- |
| `update-dashboard.mjs`       | Push full dashboard config to HA            |
| `update-main-view.mjs`       | Push main view only                         |
| `update-dev-view.mjs`        | Push dev view only                          |
| `backup-dashboard.mjs`       | Snapshot current config to `backups/`       |
| `restore-dashboard.mjs`      | Restore from a backup                       |
| `reload-templates.mjs`       | Trigger HA template reload                  |
| `generate-plant-sensors.mjs` | Regenerate `config/fyta_plant_sensors.yaml` |
| `integration-test.sh`        | Smoke-test the live dashboard               |
