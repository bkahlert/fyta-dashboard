...
ws.onmessage = (event) => {
  ...
  if (msg.type === "auth_required") {
    ...
    const autoEntities = mainView.cards.find(
      (c) => c.type === "custom:auto-entities",
    );
    if (!autoEntities) {
      console.error("✘ auto-entities card not found in main view");
      process.exit(1);
    }
    ...
    console.log(
      "✔ Rebuilt filter: sensor.*_moisture, numeric sort ascending (thirstiest first)",
    );
