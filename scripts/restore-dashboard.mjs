...
const backupPath = process.argv[2] ?? `backups/plants-dashboard-${new Date().toISOString().slice(0, 10)}.json`;
...
ws.onmessage = (event) => {
  ...
  if (msg.type === 'auth_required') send({ type: 'auth', access_token: TOKEN });
    ...
    if (!msg.success) { console.error('Restore failed:', msg.error); ws.close(); process.exit(1); }
